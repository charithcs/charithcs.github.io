      ...
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center justify-center text-center">
        ...
        <div className="h-10 sm:h-14 flex items-center justify-center mb-8 w-full">
          <TypeAnimation
            ...
            wrapper="h2"
            speed={50}
            className="text-lg sm:text-xl md:text-2xl font-semibold text-blue-300 text-center px-2"
            repeat={Infinity}
          />
        </div>
          ...
          {metrics.map((metric, index) => {
            ...
            return (
              <div
                key={index}
                className={`
                  unified-card relative p-4 sm:p-5 md:p-6 rounded-xl transition-all duration-500 transform
                  ${isActive 
              ...
              >
                <div className="flex flex-col items-center space-y-2">
                  <Icon className={`w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 ${isActive ? metric.color : 'text-white/70'} transition-colors duration-300`} />
                  <div className="text-center">
                    ...
                    <div className={`text-sm sm:text-base ${isActive ? 'text-white/90' : 'text-white/60'} transition-colors duration-300 leading-tight`}>
                      {metric.label}
                    </div>
                  </div>
                </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-7 justify-center items-center w-full px-2">
          <Button
            ...
            size="lg"
            className="btn-unified group font-semibold px-7 sm:px-9 py-4 sm:py-5 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 w-full sm:w-auto text-base sm:text-lg"
          >
            <Shield className="mr-3 sm:mr-4 h-6 w-6 sm:h-7 sm:w-7 group-hover:rotate-12 transition-transform duration-300" />
            <span className="hidden sm:inline">🛡️ Explore My Security Arsenal</span>
            <span className="sm:hidden">🛡️ Security Arsenal</span>
            <ArrowRight className="ml-3 sm:ml-4 h-6 w-6 sm:h-7 sm:w-7 group-hover:translate-x-1 transition-transform duration-300" />
          </Button>
          <Button
            onClick={() => scrollToSection('#experience')}
            variant="outline"
            size="lg"
            className="btn-glass group font-semibold px-7 sm:px-9 py-4 sm:py-5 rounded-2xl transition-all duration-300 transform hover:scale-105 w-full sm:w-auto text-base sm:text-lg"
          >
            <Eye className="mr-3 sm:mr-4 h-6 w-6 sm:h-7 sm:w-7 group-hover:scale-110 transition-transform duration-300" />
